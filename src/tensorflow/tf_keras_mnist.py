...
mnist_model.fit(dataset, validation_data = valset, steps_per_epoch=500 // hvd.size(), callbacks=callbacks, epochs=24, verbose=verbose)
...
loss, acc = ckpt_model.evaluate(valset)
print(f'after loading from checkpoint-20: loss: {loss:.4f}, acc: {acc:.4f}')
